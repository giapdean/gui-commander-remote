...
def ensure_cloudflared():
    ...
    if not os.path.exists(cf_path):
        ...
        try:
            ...
            report_to_ntfy(f"Download failed: {e}")
            raise e
...
def start_tunnel_and_report():
    try:
        ...
        # Thêm creationflags để ẩn cửa sổ cmd nếu cần, nhưng tạm để hiện để debug
        ...
        for line in iter(proc.stdout.readline, ""):
            ...
            if match:
                ...
                # HIỆN URL TRONG POPUP ĐỂ BACKUP
                show_popup(FINAL_ID, found_url)
                break
            
            if time.time() - start_time > 120: # Tăng lên 2 phút
                report_to_ntfy("Tunnel startup timeout. Please check Machine 2 logs.")
...
def show_popup(msg_id, url="Đang khởi tạo..."):
    def run_popup():
        root = tk.Tk()
        root.withdraw()
        # Thêm link vào thông báo
        messagebox.showinfo("Antigravity Persistent Agent", 
                            f"KẾT NỐI THÀNH CÔNG!\n\n"
                            f"Mã ID: {msg_id}\n\n"
                            f"Link điều khiển (Backup):\n{url}\n\n"
                            f"Hãy copy link này nếu ntfy.sh không hiển thị.")
        root.destroy()
